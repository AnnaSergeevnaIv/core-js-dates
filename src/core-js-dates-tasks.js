...
function getNextFriday(date) {
  ...
  if (dayNumber < 5) {
    additionalDays = 5 - dayNumber;
  } else if (dayNumber === 5) {
    additionalDays = 7;
  } else additionalDays = 6;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const first = Date.parse(new Date(dateStart));
  const second = Date.parse(new Date(dateEnd));
...
function getWeekNumberByDate(date) {
  ...
  const daysCount = (date - firstDateOfYear) / (1000 * 60 * 60 * 24) + 1;

  const diff = firstDayOfYear <= 4 ? firstDayOfYear - 1 : firstDayOfYear - 7;
  return Math.ceil((daysCount + diff) / 7);
}
...
function getQuarter(date) {
  const year = date.getFullYear();
  if (Date.parse(date) > Date.parse(new Date(year, 9, 0))) return 4;
  if (Date.parse(date) > Date.parse(new Date(year, 7, 0))) return 3;
  if (Date.parse(date) > Date.parse(new Date(year, 4, 0))) return 2;
  if (Date.parse(date) > Date.parse(new Date(year, 0, 0))) return 1;
  return -1;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const result = [];
  const subDay = startDate;
  while (subDay <= endDate) {
    for (let i = 1; i <= countWorkDays; i += 1) {
      if (subDay > endDate) break;
      const day = `${subDay.getDate()}`.padStart(2, '0');
      const month = `${subDay.getMonth() + 1}`.padStart(2, '0');
      const year = subDay.getFullYear();
      result.push(`${day}-${month}-${year}`);
      if (i === countWorkDays)
        subDay.setDate(subDay.getDate() + countOffDays + 1);
      else subDay.setDate(subDay.getDate() + 1);
    }
  }
  return result;
}
